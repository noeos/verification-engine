# Fuentes normativas y técnicas

Estado: **registro de referencia**
Consulta: **2026-08-26**

## Protocolo y datos

- [RFC 2119 — Requirement Levels](https://www.rfc-editor.org/rfc/rfc2119)
- [RFC 8174 — Ambiguity of Uppercase vs Lowercase](https://www.rfc-editor.org/rfc/rfc8174)
- [RFC 8785 — JSON Canonicalization Scheme](https://www.rfc-editor.org/rfc/rfc8785.html)
- [RFC 7493 — I-JSON](https://www.rfc-editor.org/rfc/rfc7493.html)
- [RFC 8259 — JSON](https://www.rfc-editor.org/rfc/rfc8259.html)
- [RFC 4648 — Base encodings](https://www.rfc-editor.org/rfc/rfc4648.html)
- [Unicode UAX #15, versión vigente](https://unicode.org/reports/tr15/)

## Criptografía

- [NIST FIPS 180-4 — Secure Hash Standard](https://csrc.nist.gov/pubs/fips/180-4/upd1/final)
- [Node.js Crypto API](https://nodejs.org/api/crypto.html)
- [Python 3.13.15](https://www.python.org/downloads/release/python-31315/)

NIST anunció revisión de FIPS 180-4; el proyecto vigila la sucesora antes de cada release.

## Desarrollo seguro y suministro

- [NIST SP 800-218 SSDF 1.1](https://csrc.nist.gov/pubs/sp/800/218/final)
- [SLSA specification 1.2](https://slsa.dev/spec/v1.2/)
- [npm trusted publishing](https://docs.npmjs.com/trusted-publishers/)
- [npm provenance](https://docs.npmjs.com/generating-provenance-statements/)
- [npm 2FA](https://docs.npmjs.com/requiring-2fa-for-package-publishing-and-settings-modification/)
- [CycloneDX 1.7](https://cyclonedx.org/specification/overview/)
- [SPDX 3.0](https://spdx.dev/use/specifications/)
- [CVSS 4.0](https://www.first.org/cvss/v4.0/specification-document)
- [Node.js release lifecycle](https://nodejs.org/en/about/previous-releases)

## Legalidad UE/España

- [Cyber Resilience Act, Reglamento (UE) 2024/2847](https://eur-lex.europa.eu/eli/reg/2024/2847/oj/eng)
- [Resumen CRA de la Comisión](https://digital-strategy.ec.europa.eu/en/policies/cra-summary)
- [CRA y open source](https://digital-strategy.ec.europa.eu/en/policies/cra-open-source)
- [Guía de aplicación CRA de la Comisión, julio de 2026](https://digital-strategy.ec.europa.eu/en/library/commission-publishes-new-guidance-support-timely-cyber-resilience-act-implementation)
- [Responsabilidad por productos, Directiva (UE) 2024/2853](https://eur-lex.europa.eu/eli/dir/2024/2853/oj/eng)
- [RGPD, Reglamento (UE) 2016/679](https://eur-lex.europa.eu/eli/reg/2016/679/oj)
- [LOPDGDD, LO 3/2018](https://www.boe.es/buscar/act.php?id=BOE-A-2018-16673)
- [AEPD — privacidad desde el diseño](https://www.aepd.es/derechos-y-deberes/cumple-tus-deberes/medidas-de-cumplimiento/proteccion-de-datos-desde-el-diseno)
- [NIS2, Directiva (UE) 2022/2555](https://eur-lex.europa.eu/eli/dir/2022/2555/oj)
- [ENS, RD 311/2022](https://www.boe.es/buscar/act.php?id=BOE-A-2022-7191)
- [Ley de Propiedad Intelectual](https://www.boe.es/buscar/act.php?id=BOE-A-1996-8930)
- [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0)

## Política de uso

Para cuestiones técnicas se priorizan estándares y documentación oficial. Para legalidad se usan textos oficiales y orientación de autoridad; blogs/resúmenes solo facilitan lectura. La versión jurídica aplicable se confirma en publicación oficial y con profesional.

Una URL viva puede cambiar sin cambiar este archivo; el registro legal conserva además fecha, versión/ELI y captura/hash en el expediente cuando sea material.
